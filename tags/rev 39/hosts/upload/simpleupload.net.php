...
<?php
			$ref='http://simpleupload.net/index.php';
			$page = geturl("simpleupload.net", 80, "/index.php", 0, 0, 0, 0, "");
			is_page($page);
?>
...
<?php		
			...
			is_page($page);
			$tmp=cut_str($page,'http://simpleupload.net/download/',"'");
			if (!$tmp) html_error ('Error get download link'.$page);
			$download_link='hhttp://simpleupload.net/download/'.$tmp;
			$tmp=cut_str($page,'http://simpleupload.net/download/delete','"');
			if ($tmp) $delete_link='http://simpleupload.net/download/delete'.$tmp;
?>
