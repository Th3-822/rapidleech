...
$not_done=true;
$continue_up=false;
if ($ravishare_login & $ravishare_pass){
	$_REQUEST['login'] = $ravishare_login;
	$_REQUEST['password'] = $ravishare_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to ravishare.com</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("ravishare.com", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("ravishare.com", 80, "/?op=my_files", "http://ravishare.com/login.html", $cookies, 0, 0, "");
			is_page($page);
