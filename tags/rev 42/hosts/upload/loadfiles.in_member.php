...
$not_done=true;
$continue_up=false;
if ($loadfiles_login & $loadfiles_pass){
	$_REQUEST['login'] = $loadfiles_login;
	$_REQUEST['password'] = $loadfiles_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to loadfiles.in</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("loadfiles.in", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("loadfiles.in", 80, "/?op=my_files", "http://loadfiles.in/login.html", $cookies, 0, 0, "");
			is_page($page);
...
// Made By CHRISTIAN™ 23/06/2010 14:10
?>
