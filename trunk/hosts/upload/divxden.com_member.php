...
$not_done=true;
$continue_up=false;
if ($divxden_login & $divxden_pass){
	$_REQUEST['login'] = $divxden_login;
	$_REQUEST['password'] = $divxden_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to divxden.com</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("divxden.com", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("divxden.com", 80, "/?op=my_files", "http://divxden.com/login.html", $cookies, 0, 0, "");
			is_page($page);
