...
$not_done=true;
$continue_up=false;
if ($qup_login & $qup_pass){
	$_REQUEST['login'] = $qup_login;
	$_REQUEST['password'] = $qup_pass;
...
<?php
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("quickupload.net", 80, "/", 'http://quickupload.net/', 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("quickupload.net", 80, "/?op=my_files", "http://quickupload.net/", $cookies, 0, 0, "");
			is_page($page);
...
<?
	...
	$Url=parse_url($ref);
	$page = geturl($Url["host"], defport($Url), $Url["path"].($Url["query"] ? "?".$Url["query"] : ""), 'http://quickupload.net/', $cookies, $gpost, 0, $_GET["proxy"],$pauth);
