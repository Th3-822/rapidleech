...
$not_done=true;
$continue_up=false;
if ($smw_login & $smw_pass){
	$_REQUEST['login'] = $smw_login;
	$_REQUEST['password'] = $smw_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to sendmyway.com</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("sendmyway.com", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("sendmyway.com", 80, "/?op=my_files", "http://sendmyway.com/login.html", $cookies, 0, 0, "");
			is_page($page);
...
<?
	...
	$download_link="http://sendmyway.com/" . $locat . "/" . $lname;
	}
	
/**************************************************\  
WRITTEN by pasolvon 05 Dec 2011
\**************************************************/
?>