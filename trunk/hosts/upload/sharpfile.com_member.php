...
$not_done=true;
$continue_up=false;
if ($sharpfile_login & $sharpfile_pass){
	$_REQUEST['login'] = $sharpfile_login;
	$_REQUEST['password'] = $sharpfile_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to sharpfile.com</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "http://sharpfile.com/" ;
			$post['login'] = $_REQUEST['login'];
			$post['password'] = $_REQUEST['password'];
			$page = geturl("sharpfile.com", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("sharpfile.com", 80, "/?op=my_files", "http://sharpfile.com/login.html", $cookies, 0, 0, "");
			is_page($page);
...
<?php
	$ref='http://sharpfile.com/?op=upload_form';
	$Url=parse_url($ref);
...
<?
		...
		if ($status=='OK') {
			...
			$download_link="http://sharpfile.com/" . $locat . "/" . $lname . ".html";
		} else {
