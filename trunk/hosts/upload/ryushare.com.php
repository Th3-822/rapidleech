<?php
####### Account Info. ###########
$upload_acc['ryushare_com']['user'] = ""; //Set your user
$upload_acc['ryushare_com']['pass'] = ""; //Set your password
##############################

$not_done=true;
$continue_up=false;
if ($upload_acc['ryushare_com']['user'] && $upload_acc['ryushare_com']['pass']) {
	$_REQUEST['login'] = $upload_acc['ryushare_com']['user'];
	$_REQUEST['password'] = $upload_acc['ryushare_com']['pass'];
...
<tr><td align="center">
<div id="login" style="width:100%;text-align:center;">Login to ryushare.com</div>
<?php
	...
	if (!empty($_REQUEST['login']) && !empty($_REQUEST['password'])) {
		...
		$page = geturl("ryushare.com", 80, "/", 'http://ryushare.com/', $cookie, $post, 0, $_GET["proxy"], $pauth);is_page($page);
...
<?php
	$page = geturl("ryushare.com", 80, "/", 'http://ryushare.com/', $cookie, 0, 0, $_GET["proxy"], $pauth);is_page($page);
...
<?php
	...
	$upfiles=upfile($url["host"], 80, $url["path"].($url["query"] ? "?".$url["query"] : ""), 'http://ryushare.com/', $cookie, $post, $lfile, $lname, "file_0");
...
<?php
	...
	$page = geturl("ryushare.com", 80, "/", $up_url, $cookie, $post, 0, $_GET["proxy"], $pauth);is_page($page);

	if (preg_match('@(https?://(?:www\.)?ryushare\.com/\w+(?:/[^\?|/|<|>|\"|\'|\r|\n]+)?(?:\.html)?)\?killcode=\w+@i', $page, $lnk)) {
		$download_link = $lnk[1];
		$delete_link = $lnk[0];
