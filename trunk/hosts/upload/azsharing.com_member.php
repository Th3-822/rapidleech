...
if ($ezy_login & $ezy_pass){
	$_REQUEST['login'] = $az_login;
	$_REQUEST['password'] = $az_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to azsharing.com</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("azsharing.com", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("azsharing.com", 80, "/?op=my_files", "http://azsharing.com/login.html", $cookies, 0, 0, "");
			is_page($page);
...
// Made by Baking 27/06/2009 22:22
?>