...
<?php
			...
			is_page($page);
                        ...
                        $tid3=rndNum(12);
			preg_match('%action="(.*)" method="post"%',$page,$match);
			$url_action="$match[1]$tid3&js_on=1&utype=reg&upload_type=file&box=1";
                        preg_match("%var srv_tmp_url='(.*)';%",$page,$match);
                        $srv_tmp_url = $match[1];
...
<?php		
			...
			$upfiles = upfile($url["host"],$url["port"] ? $url["port"] : 80, $url["path"].($url["query"] ? "?".$url["query"] : ""),"http://usershare.net/box?xbg=FFFFFF&xtxt=101010", $cookie, $fpost, $lfile, $lname, "file_1");
...
Update by VinhNhaTrang 17.11.2010
\*************************/
?>