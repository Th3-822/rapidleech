...
$exte = cut_str($lname,'.',false);
if (($exte !== 'divx') && ($exte !== 'avi')){
	html_error('Error - Only these extensions are allowed: avi,divx');
	}

$not_done=true;
$continue_up=false;
if ($metdx_login & $metdx_pass){
	$_REQUEST['login'] = $metdx_login;
	$_REQUEST['password'] = $metdx_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to metadivx.com</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("metadivx.com", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("metadivx.com", 80, "/?op=my_files", "http://metadivx.com/login.html", $cookies, 0, 0, "");
			is_page($page);
