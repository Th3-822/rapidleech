...
class filedefend_com extends DownloadClass {
	private $page, $cookie;
	public function Download($link) {
		...
		if ($_REQUEST['premium_acc'] == 'on' && ((!empty($_REQUEST['premium_user']) && !empty($_REQUEST['premium_pass'])) || (!empty($premium_acc['filedefend_com']['user']) && !empty($premium_acc['filedefend_com']['pass'])))) $this->Login($link);
	...
	private function Login($link) {
		...
		$user = ($pA ? $_REQUEST['premium_user'] : $premium_acc['filedefend_com']['user']);
		$pass = ($pA ? $_REQUEST['premium_pass'] : $premium_acc['filedefend_com']['pass']);
...
// [04-11-2012]  Written by Th3-822. (XFS, XFS everywhere. D:)
