...
class novamov_com extends DownloadClass {
	public function Download($link) {
		...
		$page = $this->GetPage("http://www.novamov.com/api/player.api.php?user=undefined&codes=1&file={$fid[1]}&pass=undefined&key={$fkey[1]}");
		if (!preg_match('@url=(http://[^\&|\r|\n]+)@i', $page, $downl)) html_error('Error: Download link not found.');
		if (!preg_match('@title=([^\&|\r|\n]+)\%26@i', $page, $fname)) html_error('Error: Video title not found.');
