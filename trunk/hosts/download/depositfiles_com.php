...
class depositfiles_com extends DownloadClass {
    ...
    public function Download($link) {
        ...
        if ($_POST['pass'] == 'premium') {
            ...
            is_present($page, 'The file\'s password is incorrect. Please check your password and try to enter it again.');
            return $this->DownloadPremium($link, $cookie, $page);
            ...
            $page = $this->GetPage($link, $cookie, $post, $Referer);
            is_present($page, 'The file\'s password is incorrect. Please check your password and try to enter it again.');
            return $this->DownloadFree($link, $page);
        } else {
            return $this->DownloadFree($link);
        }
    }

    private function DownloadFree($link, $page = false) {
        global $Referer;
        ...
        if (!$page) {
            ...
            $page = $this->GetPage($link, $cookie, array('gateway_result' => '1'), $Referer);
        ...
        if (stristr($page, 'You used up your limit for file downloading!')) {
            ...
            html_error("Download limit exceeded. Try again in " . trim($minutes [1]) . " minute(s)");
        }
        if (stristr($page, 'Please, enter the password for this file')) {
            echo "\n" . '<center><form action="' . $PHP_SELF . '" method="post" >' . "\n";
            echo '<input type="hidden" name="pass" value="free" />' . "\n";
    ...
    private function Login($link, $cap, $autolog = false) {
        ...
        return $this->DownloadPremium($link, $cookie);
    }

    private function DownloadPremium($link, $cookie, $page = false) {
        global $Referer;
