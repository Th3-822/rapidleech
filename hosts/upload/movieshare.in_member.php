...
$not_done=true;
$continue_up=false;
if ($movieshare_login & $movieshare_pass){
	$_REQUEST['login'] = $movieshare_login;
	$_REQUEST['password'] = $movieshare_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to movieshare.in</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("movieshare.in", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("movieshare.in", 80, "/?op=my_files", "http://movieshare.in/login.html", $cookies, 0, 0, "");
			is_page($page);
