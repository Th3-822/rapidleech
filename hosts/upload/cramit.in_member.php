...
$not_done=true;
$continue_up=false;
if ($cramitin_login & $cramitin_pass){
	$_REQUEST['login'] = $cramitin_login;
	$_REQUEST['password'] = $cramitin_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to cramit.in</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("cramit.in", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("cramit.in", 80, "/?op=my_files", "http://cramit.in/login.html", $cookies, 0, 0, "");
			is_page($page);
