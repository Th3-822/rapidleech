...
$not_done=true;
$continue_up=false;
if ($superfastfile_login & $superfastfile_pass){
	$_REQUEST['login'] = $superfastfile_login;
	$_REQUEST['password'] = $superfastfile_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to superfastfile.com</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("superfastfile.com", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("superfastfile.com", 80, "/?op=my_files", "http://superfastfile.com/login.html", $cookies, 0, 0, "");
			is_page($page);
