...
$not_done=true;
$continue_up=false;
if ($loomb_login & $loomb_pass){
	$_REQUEST['login'] = $loomb_login;
	$_REQUEST['password'] = $loomb_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to loombo.com</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("loombo.com", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("loombo.com", 80, "/?op=my_files", "http://loombo.com/login.html", $cookies, 0, 0, "");
			is_page($page);
