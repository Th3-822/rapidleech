...
$continue_up=false;
if ($us_login & $us_pass){
	$_REQUEST['login'] = $us_login;
	$_REQUEST['password'] = $us_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to uploadspace.eu</div>
<?php
			$post['op'] = "login" ;
			$post['redirect'] = "" ;
			...
			$post['x'] = "0" ;
			$post['y'] = "0" ;
			$page = geturl("uploadspace.eu", 80, "/login.html", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
...
<?
	...
	$upfiles=upfile($url["host"],defport($url), $url["path"].($url["query"] ? "?".$url["query"] : ""),$upsrv, $cookies, $post, $lfile, $lname, "file_0");
...
<?
	...
	$Url=parse_url($ref);
	$page = geturl($Url["host"], defport($Url), $Url["path"].($Url["query"] ? "?".$Url["query"] : ""), $ref, $cookies, $gpost, 0, $_GET["proxy"],$pauth);
	is_page($page);
...
// Member upload plugin 16/07/2009 14:39
?>