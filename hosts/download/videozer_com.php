...
class videozer_com extends DownloadClass {
	...
	private function vbb_decode($page) {
		...
		if (!$key1 = $json["cfg"]["environment"]["rkts"]) html_error("Error getting keys [1]");
		if (!$key2 = $json["cfg"]["login"]["pepper"]) html_error("Error getting keys [2]");
		if (!$key3 = $json["cfg"]["ads"]["lightbox2"]["time"]) html_error("Error getting keys [3]");

		list ($spn, $outk) = explode(";", $this->hextostr($this->bitDecrypt($json["cfg"]["login"]["spen"], $json["cfg"]["login"]["salt"], 950569)), 2);
	...
	private function getikey($int) { 
		switch($int) {
			...
			case 1:return 215678;
			case 2:return 516929;
			case 3:return 962043;
			case 4:return 461752;
			case 5:return 141994;
		}
	}
