...
class billionuploads_com extends DownloadClass {
	...
	public function Download($link) {
		...
		if (empty($_POST['step']) || $_POST['step'] != '1') {
			...
			is_present($this->page, 'incapsula.com/jsTest.html', '"CDN" challenge page found... Try again later.');
		...
		if (empty($_POST['step']) || $_POST['step'] != '1') {
			...
			if (preg_match('@https?://api\.solvemedia\.com/papi/challenge\.(?:no)?script\?k=([\w\.-]+)@i', $this->form, $skey)) {
	...
	public function _GetPage() {
		...
		if (stripos($page, 'incapsula.com/jsTest.html') !== false) {
	...
	private function loadCookie() {
		...
		$file = file($filename);
		$data = decrypt(base64_decode($file[1]));
		unset($file);
		if (!empty($data) && substr($data, 0, 6) === 'T8:OK:') {
			$this->cookie = StrToCookies(substr($data, 6));
			unset($this->cookie['aff']);
			return true;
		}
		return false;
	...
	private function saveCookie() {
		...
		file_put_contents($filename, "<?php exit(); ?>\r\n" . base64_encode(encrypt("T8:OK:$cookies")), LOCK_EX);
		return true;
	}
}
...
// [03-6-2014]  Fixed cdn-related functions. - Th3-822
