...
{
	####### Free Account Info. ###########
	private $btaccel_login = "";					//  Set your username
	private $btaccel_pass = "";						//  Set your password
	##############################
	...
	private function displayLinks( $link )
	{
		...
		{
			$this->btaccel_login = trim( $user );
			$this->btaccel_pass = trim( $pass );
			
			$link = $GetUser['scheme'] . "://" . $GetUser['host'] .$GetUser['path'];
		}
		
		$cookies = $this->loginto( $this->btaccel_login, $this->btaccel_pass );
...
<?php	
	...
	{	
		...
		else
		{
			...
			$page = $this->GetPage('http://www.btaccel.com/home/', $cookie, 0, $referer);
...
/**********************************************************	
Re-Written by Raj Malhotra based on original plugin by kaox on 16 May 2010
**********************************************************/
?>